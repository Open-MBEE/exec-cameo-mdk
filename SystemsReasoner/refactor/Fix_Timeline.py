...
StateTimelineStereotype=StereotypesHelper.getStereotype(project,"State Timeline","Mission Service Architecture Framework")
                ...
                for propE in propEs:
                    ...
                    if isinstance(blockE,Class):
                        ...
                        if StereotypesHelper.hasStereotype(blockE,eventStereotype) and e.getName()==blockE.getName():
                            ...
                            ConstAttr=filter(lambda element: element.getType()!=None and StereotypesHelper.hasStereotype(element.getType(),SUCeventSpecSt) and element.getType().getName()!="Event Constraint",csEvAttr)
                            if len(ConstAttr)!=0:
            ...
            if StereotypesHelper.hasStereotype(csElem,StereoElem):  
                ...
                for genAttr in genAttrs:
                    ...
                    if genAttr.getType()==genSvElem:#this is going to get more than one thing if not in the control service where there is only one
                        CcsElem=csElem
                        #gl.log("Do we get to this initial location??")
                    elif genAttr.getType()!= None:
                        if len(genAttr.getType().getGeneral())>0:
                            if genAttr.getType().getGeneral()[0]==genSvElem:
                                CcsElem=csElem
                        #gl.log("Do we get to the secondary location??")
#                    #gl.log(m.getName())
...
##Should be one black diamond from state timeline to system under control state variable type, and one system under control  
##Each state timeline found under a certain package should be a directed composition from the same SUC

## Verify that each state, has one associated state timeline, should be a directed composition from state timeline to state variable type

## 



#guiLog.log("jeo principle stereotype " + jPrincipleStereotype.getName())

        
def run(mode):
    try:
        SessionManager.getInstance().createSession("LouiseTest")
        [GenError,redError,StateTimelineError,EventError,GenAdd,EventAdd,StateAdd,AssocAdd,RedAdd,asscError,StAdd,AbsAdd,MultState,MultEvent,mappingReqState,AbsError,constError,constAdd,litError,litEventAdd,redefCs]=StateEventTimelineCreation(package,False)
        gl.log("--------------------------Summary ERROR Report--------------------------")
        gl.log("Generalization Errors===============>"+str(GenError))
        gl.log("Redefinition Errors===============>"+str(GenError))
        ...
        gl.log("Event Multiple Association Error====>"+str(MultEvent))
        ...
        gl.log("--------------------------Summary Addition Report--------------------------")

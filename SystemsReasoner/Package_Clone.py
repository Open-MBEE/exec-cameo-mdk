...
def createRedefDep(depowner, depclient, depsupplier):
    ...
    csPacks=filter(lambda element: isinstance(element,Package),csPackage.getOwnedElement())
    csSter = filter(lambda element: "_Stereotypes" in element.getName(), csPacks)
    if len(csSter)==0: 
    	under = filter(lambda element: "_" == element.getName(), csPacks)[0]
    	csSter = filter(lambda element: isinstance(element, Package) and "_Stereotypes" in element.getName(), under.getOwnedElement())
    	csSter = csSter[0]
    else: csSter=csSter[0]
...
def run(mode):
    ...
    try:
        ...
        for m in mapping:
            if not isinstance(m, Package) and not isinstance(m,InformationFlow):

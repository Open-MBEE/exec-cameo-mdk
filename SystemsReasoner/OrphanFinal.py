...
def blah():
    ids=selected.getID()
    gl.log("what up what up===>"+str(ids))
    return

def outPutStereo(package,sucModel):
    tls=package.getOwnedElement()
    tls=filter(lambda element: StereotypesHelper.hasStereotype(element,TimelineSt), tls)
    selects=MDUtils.getUserCheckboxSelections("Stereotype Selection for Orphan Proxy Removal", "Select Stereotype, that will replace orphan===>  "+proxy.getQualifiedName(), tls)
    return selects

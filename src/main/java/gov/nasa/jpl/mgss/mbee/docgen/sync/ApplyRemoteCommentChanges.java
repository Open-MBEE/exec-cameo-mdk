...
public class ApplyRemoteCommentChanges extends ChangeTheModel {
	...
	public ApplyRemoteCommentChanges(
			Element owner,
			Map<String,Comment> localComments,
			List<SyncedComment> added,
			List<SyncedComment> modified,
			List<SyncedComment> deleted) {
		this.owner = owner;
		...
		if (successfulAdds == 0 && successfulModifies == 0 && successfulDeletes == 0) {
			...
			if (failures > 0) {
				log(String.format("Imported comments (%d added, %d modified, %d deleted)"
						+ " - %d CHANGES FAILED",
						successfulAdds, successfulModifies, successfulDeletes, failures));
				JOptionPane.showMessageDialog(null,
						"Some changes could not be imported.\nSee messages window for details.");
			} else {
				log(String.format("Imported comments (%d added, %d modified, %d deleted)",
						successfulAdds, successfulModifies, successfulDeletes));
			}
		}
	}
	...
	private void add(SyncedComment imported) {
		...
		StereotypesHelper.setStereotypePropertyValue(c, stereotype, MODIFIED_TIMESTAMP, imported.getTimestamp());
		StereotypesHelper.setStereotypePropertyValue(c, stereotype, AUTHOR, imported.getAuthor());
		c.setOwningElement(owner);

		++successfulAdds;
		log(String.format("Added comment: [%s %s] %s",
				imported.getAuthor(), imported.getTimestamp(),
	...
	private void modify(SyncedComment imported) {
		...
		if (c == null) {
			log("Can't find local comment to apply imported modification: "
					+ imported.getId() + " "
					+ CommentUtil.truncateBody(imported.getBody()));
			++failures;
			return;
		...
		++successfulModifies;
		log(String.format("Modified comment: [%s %s] %s",
				imported.getAuthor(), imported.getTimestamp(),
				CommentUtil.truncateBody(imported.getBody())));
	}
...
//					+ CommentUtil.truncateBody(imported.getBody()));
		...
		++successfulDeletes;
		log(String.format("Deleted comment: [%s %s] %s",
				imported.getAuthor(), imported.getTimestamp(),
				CommentUtil.truncateBody(imported.getBody())));
	}

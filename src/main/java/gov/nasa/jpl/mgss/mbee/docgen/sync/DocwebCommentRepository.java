...
public class DocwebCommentRepository implements CommentRepository {
	...
	public List<SyncedComment> getComments(NamedElement document) throws CommentSyncFailure {
		...
		GetMethod m = new GetMethod(baseUrlString
				+ "/rest/documents/"
				+ urlEncode(document.getID())
				+ "/comments/"
				+ "?package=" + urlEncode(document.getName()));
		try {
	...
	@Override
	public void sendComments(NamedElement document,
...
//		Application.getInstance().getGUILog().log("Sending changes:\n" + json); // DEBUG
		...
		PostMethod m = new PostMethod(baseUrlString
				+ "/rest/documents/"
				+ urlEncode(document.getID())
				+ "/comments/changes/"
				+ "?package=" + urlEncode(document.getName()));
	...
	private JSONObject makeJsonForExport(
			List<SyncedComment> newComments,
			List<SyncedComment> modifiedComments,
			List<SyncedComment> deletedComments) {
...
//		Application.getInstance().getGUILog().log("incoming: " + json); // DEBUG
		try {
			...
			if (author == null || timestamp == null) {
				Application.getInstance().getGUILog().showError("author=" + author + " timestamp=" + timestamp); // DEBUG
				return null;
			}

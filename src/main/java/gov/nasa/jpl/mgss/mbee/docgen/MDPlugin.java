...
import gov.nasa.jpl.mgss.mbee.docgen.Configurator.Context;
...
public abstract class MDPlugin extends Plugin { 
  // TODO? implements ProjectEventListener { // see DocGenPlugin and ApplicationSyncEventSubscriber 
...
//  //private static final Map<String, Boolean> commitStates = new ConcurrentHashMap<String, Boolean>();
  ...
  /**
   * Redefine to specify menu configurations.
   * For example, the body may look something like this:
   * <pre>
   * {@code
   ...
   * copyPackageMethod =
   *   ClassUtils.getMethodForArgTypes( TestPlugin.PackageUtils.class,
   *                                    "copyPackage",
   ...
   * addConfiguration( "ContainmentBrowserContext", "", "copyPackage", 
   *                   "Package Utils", copyPackageMethod );
   * addConfiguration( "BaseDiagramContext", "Class Diagram", "copyPackage",
   *                   "Package Utils", copyPackageMethod );
   * addConfiguration( "BaseDiagramContext", "Activity Diagram", "copyPackage",
   *                   "Package Utils", copyPackageMethod );
   * }
   * </pre>
   */
   ...
   * 
   * @param context
   *          the place in the application (diagram, browser, or menu) where the
   *          action is to be available; this name should match a
   *          {@link Context} exactly.
   * @param subcontext
   *          a more specific context within that named by the {@code context}
   *          parameter; for example, the type of diagram.
   * @param actionName
   *          the label for an item in the menu (available in a window menu of
   *          the application or a right-click menu according to the context)
   *          that, when selected, invokes the {@code actionMethod}
   * @param category
  ...
  public <T extends NMAction> void nmActionMethod( ActionEvent event, Element element ) {
    T action = null;
    Constructor< T > ctor =
        (Constructor< T >)ClassUtils.getConstructorForArgs( nmActionClass, new Object[] { element } );
    try {
  ...
  public void init() {
    ...
    initConfigurations(); // Override this called method, not init() unless this
    ...
    for ( Configurator.Context context : Configurator.getContexts() ) {
      ...
      for ( Entry< String, Map< String, Map< String, MDAction > > > e : subcontexts.entrySet() ) {
        ...
        // Create a separate configurator object for each configuration since
...
//    }
  ...
  protected void addConfigurator( Configurator cfgtor,
  ...
  protected void addConfigurator( Configurator cfgtor,
    ...
    for ( int j=0; j < addMethod.getParameterTypes().length; ++j ) {
      ...
      } else if ( ConfiguratorWithPriority.class.isAssignableFrom( pType ) ||
   ...
   */
  public void
      setRegisteringBinaryImplementers( boolean registeringBinaryImplementers ) {

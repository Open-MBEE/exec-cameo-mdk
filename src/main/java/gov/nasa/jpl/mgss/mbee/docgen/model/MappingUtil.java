...
public class MappingUtil {
	...
	/**
	 * Utility for refactoring properties between the library and mission characterizations (e.g, lib
	 * properties are mandatory for mc)
	 * 
	...
	public static void refactorProperties(Element lib, Element mc, ElementsFactory ef) {
		...
		// get the property sets for the mission characterization and the library characterization
		...
		for (Property mprop: mprops.values()) {
			if (lprops.containsKey(mprop.getName().replace(DEPPREFIX, ""))) {
				if (mprop.getName().startsWith(DEPPREFIX)) {
					...
					mprops.put(mprop.getName(), mprop); // so it's not recreated in next pass
					log.log("Property undeprecated: " + mprop.getName() + " undeprecated in " + mc.getHumanName());
				}
			} else {
				if (!mprop.getName().startsWith(DEPPREFIX)) {

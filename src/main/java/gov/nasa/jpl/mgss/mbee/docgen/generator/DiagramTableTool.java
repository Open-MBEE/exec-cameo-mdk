...
/**
 * This class is copied from NoMagic's DiagramTableTool in diagramtabletool.jar in magicdraw reportwizard's extensions. 
 * Since we're not using reportwizard, the class is duplicated here for convenience
 * I think 17.0.2 has a better api for dealing with generic tables, should hunt for it
 */
	...
	private GenericTableSettings genericTableSettings;
	  ...
	  public String getColumnName(Diagram tableDiagram, String columnId)
	  {
	    ...
	    PropertyColumn[] propertyColumns = this.genericTableSettings.getPropertyColumns(table, 
	      Arrays.asList(new String[] { columnId }));
	  ...
	  {
	    DiagramPresentationElement tableDiagramPresentation = Project.getProject(tableDiagram).getDiagram(tableDiagram);
	  ...
	  public Object getCellValue(Diagram tableDiagram, Element rowElement, String columnId)
	  {
	    ...
	    DiagramTableRow diagramTableRow = (DiagramTableRow)table.getRows().get(elementRowNumber.intValue() - 1);
	  ...
	  private class ElementRowNumberCache
	  {
	    ...
	    public Integer getElementRowNumber(Table table, Element rowElement)
	    {
	      ...
	      return (Integer)this.elementRowNumberMap.get(rowElement);
	    }
	  }
}

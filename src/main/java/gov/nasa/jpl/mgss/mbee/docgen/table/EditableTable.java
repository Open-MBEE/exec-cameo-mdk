 ...
 * <p>Takes a table model consisting of Objects. Objects can be Strings or magicdraw Elements. All rows in the model must have the same number of columns</p>
 * <p>Takes a list of strings for headers. number of headers must equal number of columns</p>
 * <p>Takes a model of boolean indicating which of the objects in the model should be editable. (this has no relation to whether the element itself is actually editable, that will be checked by the table when being edited)</p>
 * <p>Takes a model of PropertyEnum indicating whether the name (or value in the case of magicdraw Property) should be shown/edited</p>
 * <p>The Objects for the table model and headers must be present, editable and propertyEnum can be omitted. By default, if there's no instruction, names will be shown/edited for NamedElements and nothing can be edited</p>
 * @author dlam
 *
 */
...
public class EditableTable extends JDialog {
	...
	private static 			String	csvSeparator = ",";	// Keep history of selected separator

	/**
	 * 
	 * @param title Title of the table
	 * @param m the table model, Object can be String or magicdraw elements, all rows must have the same amount of columns
	 * @param headers list of strings, size must match number of columns
	 * @param editable can be null, whether each cell should be editable. (this does not mean whether the element is actually editable, that'll be checked when actually being edited)
	 * @param e can be null, what to show/edit for each cell. Only two choices right now, name for NamedElements or value for Properties.
	 * @param precision can be null, this is applicable to if you want to show values on properties and they're floating numbers, the precision indicates how mnay decimal places to show
	 */
	public EditableTable(String title, List<List<Object>> m, List<String> headers, List<List<Boolean>> editable, List<List<PropertyEnum>> e, Integer precision) {
	...
	/**
	 * Instead of having to make a n x m table of booleans, if your columns are either editable or not (usually the case), use this to set whether columns are editable. instance the table with null and then use this to set column wide 
	 * @param editableCol
	 */
	...
	/**
	 * Instead of having to make a n x m table of what to show, specify by columns instead. you can pass in null when instancing the table and then use this to set column wide
	 * @param whatToShowCol
	 */
	...
	public void prepareTable() {
		EditableTableModel tmodel = new EditableTableModel(model, headers, editable, whatToShow, editableCol, whatToShowCol, precision);
    	jtable = new JTable(tmodel);
    	jtable.setDefaultRenderer(String.class, new ChangedTableCellRenderer(tmodel));
    	jtable.getTableHeader().setPreferredSize(new Dimension(jtable.getColumnModel().getTotalColumnWidth(), 45));
						...
						if (e.isEditable()) {
							try {	
								...
								if (e instanceof Property && whatToChange == PropertyEnum.VALUE) {	
									if (((Property)e).getDefault() == null || !((Property)e).getDefault().equals(value)) {
										if (value.equals("")) {
											int ok = JOptionPane.showConfirmDialog(null, "You're about to set the value of " + ((NamedElement)e).getQualifiedName() + " to empty, are you sure?", 
											...
											// only mark cell as changed if value changed
											if (!curValue.equals(value)) {
												gl.log("changing table element [" + row + "][" + col + "] " + ((NamedElement)e).getQualifiedName() + "'s value from {" + curValue + "} to {" + value + "}" );
												Utils.setPropertyValue((Property)e, value);
												ntable.setElementChanged((Element)e);
											}
										}
									...
									if (!((NamedElement)e).getName().equals(value)) {
										if (value.equals("")) {
											int ok = JOptionPane.showConfirmDialog(null, "You're about to set the name of " + ((NamedElement)e).getQualifiedName() + " to empty, are you sure?", 
													"Confirm", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
											if (ok == JOptionPane.YES_OPTION) {
											...
											// only mark cell as changed if value changed
											if (!curValue.equals(value)) {
												gl.log("changing table element [" + row + "][" + col + "] " + ((NamedElement)e).getQualifiedName() + "'s name from {" + curValue + "} to {" + value + "}" );
												((NamedElement)e).setName(value);
												ntable.setElementChanged((Element)e);
											}
							...
							if (!notEditableErrorSeen) {
								JOptionPane.showMessageDialog(null, "Element " + ((NamedElement)e).getQualifiedName() + " is not editable! (Further similar errors can be seen in the log");	
								notEditableErrorSeen = true;
							}
							gl.log("Element " + ((NamedElement)e).getQualifiedName() + " is not editable!");	
						}
					} else {
						if (!positionErrorSeen) {
							JOptionPane.showMessageDialog(null, "Element with ID " + props[c] + " is not in the same position as what's in the table! (See log for further similar errors)");
							positionErrorSeen = true;
						}
						gl.log("Element with ID " + props[c] + " is not in the same position as what's in the table!");
						if (el instanceof NamedElement) {	
							gl.log("Table element: " + ((NamedElement)el).getQualifiedName() + ", Importing element: " + ((NamedElement)e).getQualifiedName());
						}
					}
					} else {
	...
	/**
	 * this may be called after prepareTable is called, if you want the jtable to mess with
	 * @return
	 */
	...
	public static String getSeparator() {
		String separator = JOptionPane.showInputDialog("Provide separator (use only a single character).", csvSeparator);

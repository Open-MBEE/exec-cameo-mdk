...
public class EditableTableModel extends AbstractTableModel {
  ...
  public EditableTableModel(List<List<Object>> m, List<String> headers,
                            List<List<Boolean>> editable,
                            List<List<PropertyEnum>> e,
                            List<Boolean> editableCol,
                            List<PropertyEnum> whatToShowCol, int precision) {
		this.model = m;
	...
	public void setValueAt(Object value, int row, int col) {
		...
		if (element instanceof Element && !((Element)element).isEditable()) {
			JOptionPane.showMessageDialog(null, "Element " + ((NamedElement)element).getQualifiedName() + " is not editable!");

...
/**
 * Responds to changes in document comments.
 * Could theoretically be implemented as a PropertyChangeListener instead of
 * TransactionCommitListener, but it's easier to get the right undo/redo
 * behavior this way. As a TransactionCommitListener, its changes are made
 * within the same session/transaction as the triggering change.
 */
public class CommentChangeListener implements TransactionCommitListener {
	/**
	 * Allow listener to be disabled during imports,
	 * to prevent automatic update of timestamps.
	 */
	...
	/**
	 * Adapter to call handleChangeEvent() from the
	 * TransactionCommitListener interface. 
	 */
...
//			String debug = e.getSource().getClass().getName() + " " + e.getPropertyName() + ": "
//					+ e.getOldValue().getClass().getName()
//					+ e.getOldValue() + " -> " + e.getNewValue() + " [" + e.getPropagationId() + "]";
//			return debug;
//		}
	}
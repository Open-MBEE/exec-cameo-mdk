...
public class CharacterizationTreeTableModel extends AbstractTreeTableModel {
	...
	public String getColumnName(int column) {
		if (column == 0)
			...
			String characterizationName = characterizations.get(column - 2)
					.getName();

			return "<html>" + characterizationName.replaceAll(" ", "<br/>")
					+ "</html>";
		} else {
			return "Unknown";
		}
	...
	public Object getValueAt(Object node, int column) {
		...
		if (column == 0)
			...
			return missionNode.getData().hasLibraryCharacterization(
					characterizations.get(column - 2));
		} else
			return "Unknown";
	}
	...
	public Object getChild(Object node, int index) {
		...
		List<Node<String, MissionComponent>> childList = libraryNode
				.getChildrenAsList();
	...
	public boolean isCellEditable(Object node, int column) {
		MissionComponent com = ((Node<String, MissionComponent>) node)
				.getData();
		if (column == 1)
			return false;
		if (column > 1) {
	...
	public void setValueAt(Object value, Object node, int column) {
		...
		NamedElement libraryCharacterization = characterizations
				.get(column - 2);
		if (checked)
			component
					.createAndAddMissionCharForLibChar(libraryCharacterization);
		else
			component
					.removeCharacterizationForLibraryChar(libraryCharacterization);
	}
}

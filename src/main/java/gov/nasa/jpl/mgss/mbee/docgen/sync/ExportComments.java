...
public class ExportComments extends MDAction {
	...
	public void actionPerformed(ActionEvent ac) {
		...
		ic.actionPerformed(null); //this is a band-aid to force an import before an export because of a bug
		//if ppl don't import comments before exporting those comments can get deleted on docweb
...
//log.log("Local checksum " + CommentUtil.checksum(c, stereotype) + " " + CommentUtil.truncateBody(c.getBody())); // DEBUG
		}
...
//log.log("Remote ID: " + id + " " + CommentUtil.truncateBody(c.getBody())); // DEBUG
				} else {
					remoteCommentsWithoutId.put(CommentUtil.checksum(c), c);
//log.log("Remote checksum: " + CommentUtil.checksum(c) +" " + CommentUtil.truncateBody(c.getBody())); // DEBUG
				}
			}
			...
			for (Comment local: localComments.values()) {
				...
				if (remote == null) {
//log.log("Local comment not found in remotes by ID: " +local.getID() +" " + CommentUtil.truncateBody(local.getBody())); // DEBUG
...
//log.log("Local comment not found in remotes by checksum: " + CommentUtil.checksum(converted) +" " + CommentUtil.truncateBody(converted.getBody())); // DEBUG
			...
			for (SyncedComment remote: remoteCommentsWithoutId.values()) {
				if (!remote.isDeleted() &&
						!localCommentsByChecksum.containsKey(CommentUtil.checksum(remote))) {
					deleted.add(remote.deletedClone());
				}
			}
			...
			if (added.isEmpty() && modified.isEmpty() && deleted.isEmpty()) {
				...
				log.log(String.format("Exported comments (%d added, %d modified, %d deleted)",
			...
			if (repository != null) // cleanup
				try { repository.close(); } catch (CommentSyncFailure e) { /* IGNORE */ }
		}
	}

	/**
	 * Verify that the project is committed to TeamWork (or saved for local projects).
	 * Allow user to proceed anyway after a warning, if they promise to be careful.
	...
	private boolean okToProceed() {
		...
		return JOptionPane.OK_OPTION == JOptionPane.showConfirmDialog(
				...
				"Project has " + dirtyWord + " changes.\nDo you want to continue?",
				"Warning", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
	}

	private SyncedComment convert(
			Comment comment,
			Stereotype stereotype,
			SyncedComment.Action action) {
		return new SyncedComment(
				comment.getID(),
				CommentUtil.author(comment, stereotype),
				CommentUtil.timestamp(comment, stereotype),
				comment.getBody(),
				action);
	}

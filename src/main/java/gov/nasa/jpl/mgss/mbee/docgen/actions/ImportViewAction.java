...
public class ImportViewAction extends MDAction {
	...
	public static void doImportView(Element e, boolean willchange, Boolean recursive, String curl) {
		...
		ValidationRule notLocked = new ValidationRule("Not Editable", "Element is not locked", ViolationSeverity.ERROR);
		ValidationRule docChange = new ValidationRule("Doc change", "Element documentation is changed", sev);
		ValidationRule valueChange = new ValidationRule("Value change", "Property's default value is changed", sev);
	...
	public static JSONArray change(String json, boolean willchange, ValidationRule nameChange, ValidationRule notLocked, ValidationRule docChange, ValidationRule valueChange) {
		JSONArray res = new JSONArray();
		JSONArray changed = (JSONArray)JSONValue.parse(json);
		GUILog gl = Application.getInstance().getGUILog();
		Project p = Application.getInstance().getProject();
		CommentChangeListener.disable(); //if the documentation of element is changed, we don't want the listener to add any stereotypes to the comment representing documentation
		try {
			...
			for (Object change: changed) {
				if (change instanceof JSONObject) {
					...
					if (mdelement == null) {
						gl.log("[ERROR] The element with id " + mdid + " is no longer in the model - the web version is probably out of date.");
						notfound++;
						continue;
					}
				/*	if (!mdelement.isEditable()) {
						if (mdelement instanceof NamedElement)
							gl.log(((NamedElement)mdelement).getQualifiedName() + " is not editable!");
						else
							gl.log("the element with id " + mdid + " is not editable!");
						cannot++;
						continue;
					}*/
					if (mdelement instanceof Element) {
						...
						if (mdelement instanceof NamedElement) {
							...
							if (!curName.equals(name)) {
								if (!mdelement.isEditable()) {
									notLocked.addViolation((Element)mdelement, "Name can't be changed - not editable");
									editable = false;
								} else {
						...
						if (mdelement instanceof Property && element.containsKey("dvalue")) {
							if (!((Property)mdelement).getDefault().equals((String)element.get("dvalue"))) {
								if (!mdelement.isEditable()) {
									notLocked.addViolation((Element)mdelement, "Default value can't be changed - not editable");
									editable = false;
								} else {
									valueChange.addViolation((Element)mdelement, "Default value change to " + (String)element.get("dvalue"));
									if (willchange)
										Utils.setPropertyValue((Property)mdelement, (String)element.get("dvalue"));
									echanged = true;
								}
							}
						}
            if (mdelement instanceof Slot && element.containsKey("dvalue")) {
              if (!Utils.slotValueToString((Slot)mdelement).equals((String)element.get("dvalue"))) {
                if (!mdelement.isEditable()) {
                  notLocked.addViolation((Element)mdelement, "Slot value can't be changed - not editable");
                  editable = false;
                } else {
                  valueChange.addViolation((Element)mdelement, "Slot value change to " + (String)element.get("dvalue"));
                  if (willchange)
                  ...
                  echanged = true;
						...
						if (!curdocwithouthtml.equals(doc)) {
							if (!mdelement.isEditable()) {
								notLocked.addViolation((Element)mdelement, "Doc or comment body can't be changed - not editalbe");
								editable = false;
							} else {
								if (mdelement instanceof NamedElement)
								...
								if (dochtml.equals(""))
									dochtml = " "; //this is to prevent a documentation getting set to "", then md will set an annotated comment to its documentation
								if (willchange) {
									if (mdelement instanceof Comment)

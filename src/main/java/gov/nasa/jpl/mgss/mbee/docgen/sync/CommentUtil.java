...
public class CommentUtil {
	...
	/** Basically ISO 8601, no milliseconds, local timezone (yyyy-mm-dd' 'hh:mm:ss) */
	...
	/**
	 * Use this to correlate comments that were created remotely without ID
	 * with their local counterparts. This handles the situation where
	 * we don't have the remote ID. For example, maybe a comment is created
	 * remotely, imported, and then exported again. Or maybe a remote comment
	 * is imported twice; we only want to add it once to the local comments.
	 * 
	 * @param c either local comment or converted from remote comment
	 * @return
	 */
	 ...
	 * is imported twice; we only want to add it once to the local comments.
	 * 
	 * @param c local comment
	 * @param s stereotype for DocumentComment, so we don't have to retrieve it
	 * @return
	 */
	...
	public static String truncateBody(String body) {
		...
		plaintext = plaintext.replaceFirst("</\\s*html.*$", ""); // could have html element without body

...
public class GeneratePDF extends MDAction {
    ...
    protected File askForPDFFile() {
    	File outputPdfFile = fileSelect("Specify a pdf file to save ...", new File(pdfDefaultDir, view.getHumanName().substring(view.getHumanType().length()).trim() + ".pdf"), "Save", new FileNameExtensionFilter("PDF", "pdf"));
    ...
    protected void generate(File autoGeneratedDocbookfile, File docbookXslFo, File outputPdfFile) {
    	...
    	if ( doc != null)
			ProgressStatusRunner.runWithProgressStatus(new DocumentAndPDFWriter(doc, doc.getGenNewImage(), autoGeneratedDocbookfile, docbookXslFo, outputPdfFile, pluginDirectory), "DocGen", true, 0);

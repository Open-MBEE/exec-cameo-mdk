...
public class MDUtils {
  ...
  public static DiagramPresentationElement getActiveDiagram( boolean complain ) {
    try {
      ...
      if ( project != null ) {
        // Debug.outln( EmfUtils.spewContents( (Object)project, 0, 3, false ) );
  ...
  public static Collection< Element > getSelectionInContainmentBrowser() {
    ...
    if ( tree == null ) return coll;
    Node[] nodes = tree.getSelectedNodes();
    if ( Utils2.isNullOrEmpty( nodes ) ) return coll;
    coll = new ArrayList< Element >();
    for ( Node selectedNode : nodes ) {
      Object selected = (Element)selectedNode.getUserObject();
  ...
  public static Collection< Element > getSelectionInDiagram() {
    ...
    if ( selectedList != null ) {
      for ( PresentationElement pe : selectedList ) {
        ...
        if ( e != null ) selectedElements.add( e );
      }
    }
...
//      ActionsMenuCreator.CustomJMenuItem item = (ActionsMenuCreator.CustomJMenuItem)source;
...
//  public gov.nasa.jpl.ae.event.Expression<?> toAeExpression( ValueSpecification mdValueSpec ) {
//    Assert.assertFalse( true ); // TODO
//    return null;
//  }
//  
//  public gov.nasa.jpl.ae.event.Expression<?> toAeExpression( Expression mdExpression ) {
//    Assert.assertFalse( true ); // TODO

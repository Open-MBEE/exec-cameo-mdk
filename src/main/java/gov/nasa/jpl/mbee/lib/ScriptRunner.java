  ...
  /**
	...
	public static Object runScriptFromStereotype(Element e, Stereotype s, Map<String, Object> addInputs) throws ScriptException {
		...
		if (queries == null || queries.isEmpty()) {
		  // For backward compatibility, also try Queries, the former name for Expose.
	 ...
	 * @param language
	 * @param inputs A map of key/value pair of script input (it can be anything really, as long as the script knows what to do with it. the inputs object will be passed to the script as 'scriptInput'
	 * @param script File of the script file
	 * @param binDir File of the script directory 
	 * @return a var called scriptInput will be accessible in the script, this is a map of key value pairs, keys will be based on what the script does and what the corresponding stereotype tags in md are, 
	 * to return something from the script, assign a map to scriptOutput var in your script
	 * @throws ScriptException 
	 */
	public static Object runScript(String language, Map<String, Object> inputs, File script, File[] binDirs) throws ScriptException {
		...
		try {
			...
			urls[count] = (new File(ApplicationEnvironment.getInstallRoot() + File.separator + "plugins" + File.separator + "com.nomagic.magicdraw.jpython" + File.separator + "jython2.5.1" + File.separator + "Lib")).toURI().toURL();
			...
			if (language.equals("qvt")) {
				...
				//sc.setAttribute(ACTION_EVENT, event, ScriptContext.ENGINE_SCOPE);
			...
			e.printStackTrace();
			throw e; //if session is managed by some caller, they need to know the script failed so they can manage their sessions
		} finally {

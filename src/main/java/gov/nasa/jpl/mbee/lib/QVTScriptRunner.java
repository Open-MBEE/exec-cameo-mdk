...
/**
 * Modified from Nick's QVTScriptRunner (as of 4/20/2013) to work with DocGen, instead of extending Groovy,
 * users don't need a groovy script but just needs the qvto and qvtox compiled file in
 * DocGenUserScript directory, similar to how jython or groovy user scripts are done.
 * DocGenTargets are set as "selectedElements" input, the model itself is passed as "currentModel",
 * any other stereotype tags are passed as configuration properties.
 * 
 * Original description from Nick:
 * This class extends Groovy's Script class to provide reusable mechanisms 
 * for launching QVTO transformations as JSR233-compatible scripts.
 * 
 * <p>
 * There are two important conventions:
 * 
 * 1) A QVTO transformation <T> must be located as a file below a relative path 
 * specified by: getQVTORelativeDirectoryPathSegments().
 * By default, getQVTORelativeDirectoryPathSegments() = "transforms".
 * 
 * If you need to change this convention, override the method to specify 
 * a different relative path to search for QVTO transformation files.
 * @see com.nomagic.magicdraw.qvt.QVTOLaunchScript.getQVToTransformationScript()
 * 
 * 2) The name of the QVTO transformation that a Groovy script specializing QVTOLaunchScript 
 * must have the same basename as the groovy script.
 * That is, a Groovy script <T>.groovy will match a QVTO transformation: 
 ...
 * 
 * @see com.nomagic.magicdraw.qvt.QVTOLaunchScript.configureTransformationParameters()
 * 
 * If the QVTO transformation has "out" parameters,
 * the following method must be defined:
 * 
 * @see com.nomagic.magicdraw.qvt.QVTOLaunchScript.saveOutputParameters()
 * 
 * In some special cases, it may be useful to override the
 * following methods:
 * 
 * configureQVTEngineExecutionContext();
 * createResourceSet();
 * 
 * In the MD environment, the MD automaton and qvt plugins are loaded in MD's default plugin classloader
 * There are problems accessing clases defined in the qvt plugin; why, this is a real classloader mystery.
 * @see https://support.nomagic.com/browse/MDUMLCS-2667
 * 
 * DMME-488: Cleanup ResourceSet mess in QVTO
 * https://imce-env.jpl.nasa.gov/jira/browse/DMME-488
 * Add support for extending the Catalog-resolving ResourceSet with a RewriteURI CatalogEntry.
 * Add support for extending the Catalog-resolving ResourceSet with a dynamically-defined, javaless EPackage (metamodel, profile, schema)
 * 
...
public class QVTScriptRunner {
	...
	/**
	 * @pre this.getProperty("javax.script.filename") must be the path to the Groovy script.
	 * 
	...
	/**
	 * Use this attribute to set the values for each QVTO transformation parameter.
	 * In general, if a transformation T has parameters: T(&lt;direction> &lt;parameter>:&lt;type>, ...)
	 * then the Groovy launch script for T must configure each parameter as follows:
	 * qvtoEngine.put("&lt;parameter>", &lt;value>);
	 * <p>
	 * &lt;value> can be one of the following:
	 * <p>
	 * <ul>
	 * <li>"md:model" if the parameter value is intended to get to the currently active MagicDraw project model;</li>
	 * <li>"md:currentElement" if the parameter value is intended to get the currently selected element in a diagram (if any) or in the browser (if any), or none.
	 * Note that "md:currentElement" will get the first selected element if multiple elements are selected.</li>
	 * <li>"md:currentSelection" if the parameter value is intended to get the collection of all selected elements in the diagram (if any) and in the browser (if any), or none.</li>
	...
	public ScriptEngine createQVTOScriptEngine() {
		...
		if (null == qvtoEngine) {
			throw new RuntimeException("QVTPlugin -- could not find JSR233 engine for: " + QVTScriptEngineFactory.SHORT_NAME);
		}
	...
	/**
	 * @pre null != qvtoEngine
	 * @post result = qvtoEngine.getContext().getAttribute(QVTEngineConstants.QVTO_EXECUTION_CONTEXT, ScriptContext.ENGINE_SCOPE)
	 * 
	 * Configures the qvtoEngine's execution context with a QVTo log routed to MagicDraw's GUILog (i.e. message window)
	 * Override this method to remove the QVTo log or route it elsewhere.
	 */
...
//put in non model stereotype tag values here as config params - to qvt config properties 
		...
		sc.setAttribute(QVTEngineConstants.QVTO_SCRIPT_ENGINE_LOG_FACTORY_NAME, "com.nomagic.magicdraw.qvt.proxy.QVTScriptingEngineLogFactory", ScriptContext.ENGINE_SCOPE);
		sc.setAttribute(QVTEngineConstants.QVTO_SCRIPT_EXECUTION_JUNIT_RESULT_LOG_FACTORY_NAME, "com.nomagic.magicdraw.qvt.proxy.QVTScriptingExecutionJUnitResultLogFactory", ScriptContext.ENGINE_SCOPE);
		return context;
	}
	...
	protected File findGeneratedBundleDirectory(File dir) {
		...
		generatedDir.listFiles(new FilenameFilter() {
			public boolean accept(File d, String name) {
				if (name.matches("[1-9][0-9]+") || "latest".equals(name) || "lastSuccessfulBuild".equals(name)) {
	...
	public void parseCatalog(URL catalogURL) {
		try {
			...
			throw new QVTORuntimeException("Failed to parse catalog: " + catalogURL + " -- " + e.getMessage(), e);
		}
	}
	...
	/**
	 * @param catalogRelativePath based on <install.root>/ontologies; e.g., QVTOLaunchScript.IMCE_LOCAL_CATALOG
	 */
	...
	public void createResourceSet() {
		if (rs == null) {
			try {
				...
				throw new QVTORuntimeException("Cannot create the catalog-resolving ResourceSet: " + e.getMessage(), e);
			}
		}
	 ...
	 * @throws QVTORuntimeException if there is no such resource set.
	 */
	 ...
	 * 
	 * <rewriteURI uriStartString="[fromURIStartString]" rewritePrefix="[toRewritePrefix]" />
	 * 
	...
	public void addRewriteURICatalogEntry(String fromURIStartString, String toRewritePrefix) {
		...
		try {
			...
			throw new QVTORuntimeException("addRewriteURICatalogEntry(uriStartString='" + fromURIStartString + "', rewritePrefix='" + toRewritePrefix + "') : " + e.getMessage());
		}
	...
	public EPackage loadEPackage(String packageURI) {
		...
		if (contents.size() != 1)
			throw new QVTORuntimeException("loadEPackage(packageURI='" + packageURI + "') : the Resource should have a single toplevel EPackage!");
	...
	public URIConverter getResourceSetURIConverter() {
		if (null == rs)
			throw new QVTORuntimeException("The ResourceSet is not created yet; perhaps this method is called before createResourceSet()");
	...
	public Object run() {
		...
		
		{
			...
			ExecutionContext executionContext = (ExecutionContext) sc.getAttribute(QVTEngineConstants.QVTO_EXECUTION_CONTEXT, ScriptContext.ENGINE_SCOPE);
			if (! context_.equals(executionContext))
				throw new QVTOTransformationURIException("QVTOLaunchScript: post-condition violation in configureQVTEngineExecutionContext()", transformationURI);
		}
	...
	/*	ProgressStatusRunner.runWithProgressStatus(new RunnableWithProgress() {
			public void run(final ProgressStatus progressStatus) {
				try {
					final EvaluationMonitor qvtoEvaluationMonitor = new EvaluationMonitor() {
					...
					final IProgressMonitor qvtoCompilationMonitor = new IProgressMonitor() {

						boolean canceled = false;
						Stack<String> compilationUnits = new Stack<String>();
						Stack<Long> compilationStartTimes = new Stack<Long>();
						...
						@Override
						public void beginTask(String compilationUnit, int workEstimate) {
						}
						
						@Override
						public void subTask(String compilationUnit) {
							compilationUnits.add(compilationUnit);
							progressStatus.setDescription(SUBTASK_PREFIX + " [depth=" + compilationUnits.size() + "] " + compilationUnit);
						...
						public void worked(int worked) {
							...
							Long startTime = compilationStartTimes.pop();
							Long delay = stopTime - startTime;
							String compilationUnit = compilationUnits.pop();
							String report = String.format(WORKED_PREFIX + " took %tMm %tSs %tms for: %s", delay, delay, delay, compilationUnit);
							showMessage(report);
						}
					...
					qvtoEngine.put(QVTEngineConstants.QVTO_PROGRESS_STATUS, progressStatus);
					qvtoEngine.put(QVTEngineConstants.QVTO_EVALUATION_MONITOR, qvtoEvaluationMonitor);
					qvtoEngine.put(QVTEngineConstants.QVTO_COMPILATION_MONITOR, qvtoCompilationMonitor);
					ScriptContext sc = qvtoEngine.getContext();
					ExecutionContextImpl executionContext = (ExecutionContextImpl) sc.getAttribute(QVTEngineConstants.QVTO_EXECUTION_CONTEXT, ScriptContext.ENGINE_SCOPE);
	...
	/**
	 * This method must be explicitly defined in a Groovy script to launch a QVTO transformation T.
	 * This method must provide a value for each of T's parameters, 
	 * regardless of the QVTO transformation parameter direction (i.e., in, inout, out).
	 * 
	...
	protected URI chooseDirectoryForSavingModelResource(String modelResourceDescription, String modelResourceFilename) {
		File directory = chooseDirectory("Select a directory to save " + modelResourceDescription);
		if (directory == null) {
			throw new CanceledQVTOTransformationException(transformationURI);
		}
		
		URI modelFileResourceURI = URI.createFileURI(directory.getAbsolutePath() + File.separator + modelResourceFilename);
	...
	protected URI chooseModelResourceFile(String modelResourceDescription, String modelResourceFileExtension) {
		...
		final String dotFileExtension = (modelResourceFileExtension.startsWith(".") ? modelResourceFileExtension : "." + modelResourceFileExtension);
		JFileChooser fileDialog = new JFileChooser(dir);
		fileDialog.setFileFilter(new FileFilter() {
	...
	protected EList<EObject> chooseAndLoadModelResourceContents(String modelResourceDescription, String modelResourceFileExtension) {
		URI modelFileResourceURI = chooseModelResourceFile(modelResourceDescription, modelResourceFileExtension);
	...
	/**
	 * 
	 * @param configurationPropertyName the name of a QVTO configuration property
	 * @return
	 */
	protected Object getConfigurationPropertyValue(String configurationPropertyName) {
		ScriptContext sc = qvtoEngine.getContext();
		QvtOperationalEvaluationEnv evaluationEnv = (QvtOperationalEvaluationEnv) sc.getAttribute(QVTEngineConstants.QVTO_EVALUATION_ENV, ScriptContext.ENGINE_SCOPE);
		TransformationInstance transf = (TransformationInstance) evaluationEnv.getValueOf(QvtOperationalEnv.THIS);
		...
		if (null == configurationPropertyFeature)
			throw new QVTOTransformationURIException("QVTEngine (configuration property: " + configurationPropertyName + ") -- not found", transformationURI);
	...
	/**
	 * @param configurationPropertyName the name of a QVTO configuration property
	 * @param defaultValue the string value returned if the configuration property has no value
	 * @return the string-value of configurationPropertyName (if not null), defaultValue otherwise
	 * @throws IllegalArgumentException if the value of configurationPropertyName is not a string
	 */
	protected String getConfigurationPropertyStringValueOrDefault(String configurationPropertyName, String defaultValue) {
		...
		throw new IllegalArgumentException("no string-valued configuration property: " + configurationPropertyName + " (got a " + value.getClass().getCanonicalName() + ")");
	}
	
	/**
	 * Override this method if the transformation has INOUT or OUT parameters that need to be saved as resources.
	 * If the transformation has INOUT parameters corresponding to MagicDraw elements in the model, such parameters do not need to be saved.
	 */
	protected void saveOutputParameters() {
	}

	/**
	 * Save the model resource corresponding to a QVTO transformation INOUT or OUT parameter as a resource file.
	 * 
	 * For registering a metamodel-specific file extension:
	 * @see com.nomagic.magicdraw.qvt.QVTOUtils.addExtensionMap(Mapping)
	 * 
	 * @param outputParameterName must be a non-null string corresponding to a QVTO transformation INOUT or OUT parameter.
	 * @param outputResourceNameConfigProperty must be a non-null string corresponding to a QVTO transformation configuration property whose value will be the name of the output resource.
	 * @param extension 	must be non-null string corresponding to a registered extension for the output resource metamodel.
	 * @return the resource containing the model extent correpsonding to the QVTO output parameter.
	 * @throws IOException 
	 */
	public Resource saveOutputExtent(String outputParameterName, String outputResourceNameConfigProperty, String extension, String contentType) throws IOException {
		...
		if (!(outputResourceNameValue instanceof String))
			throw new QVTOTransformationURIException("QVTEngine (output parameter: " + outputParameterName + ") -- no value for configuration property: " + outputResourceNameConfigProperty, transformationURI);
		...
		if (null == outputResourceFile) {
			showError("QVTPlugin: no file saved for resource corresponding to output parameter: " + outputParameterName);
			return null;
		}
	...
	/**
	 * Save the model resource corresponding to a QVTO transformation INOUT or OUT parameter as a URI resource.
	 * 
	 * @param outputParameterName must be a non-null string corresponding to a QVTO transformation INOUT or OUT parameter.
	 * @param outputURI the URI of the resource whose contents will be the contents of the model resource corresponding to the named QVTO transformation output parameter
	 * @return the resource containing the model extent correpsonding to the QVTO output parameter.
	 * @throws IOException 
	 */
	public Resource saveOutputExtent(String outputParameterName, URI outputURI, String contentType) throws IOException {
		Object result = qvtoEngine.get(outputParameterName);
		if (!(result instanceof List<?>))
			throw new QVTOTransformationURIException("QVTEngine (unknown output parameter: " + outputParameterName + ") -- output artifact URI: " + outputURI, transformationURI);

		@SuppressWarnings("unchecked")
		List<EObject> outObjects = (List<EObject>) result;
		Resource r = rs.createResource(outputURI, contentType);
		if (null == r) {
			showError("QVTEngine (output parameter: " + outputParameterName + ") -- No output resource created for uri: " + outputURI);
			return null;
		}

		saveContents(r, outObjects);

		showMessage("QVTPlugin: resource corresponding to output parameter: " + outputParameterName + " saved as: " + outputURI);
		
		return r;
	}
	
	public void exportOutputOntologyExtent(String outputParameterName, String ontologyPrefixConfigurationProperty) throws IOException {
		Object result = qvtoEngine.get(outputParameterName);
		if (!(result instanceof List<?>))
			throw new QVTOTransformationURIException("QVTEngine (unknown output parameter: " + outputParameterName + ")", transformationURI);
		
		@SuppressWarnings("unchecked")
		List<EObject> outObjects = (List<EObject>) result;
		if (outObjects.size() != 1)
			throw new QVTOTransformationURIException("QVTEngine (output parameter: " + outputParameterName + ") -- there should be one output element instead of " + outObjects.size(), transformationURI);
		EObject outObject = outObjects.get(0);
		if (!(outObject instanceof Ontology))
			throw new QVTOTransformationURIException("QVTEngine (output parameter: " + outputParameterName + ") -- there should be one output ontology; got instead a " + outObject.eClass().getName(), transformationURI);
		...
		if (null == ontologyModelURI)
			throw new QVTOTransformationURIException("QVTEngine (output parameter: " + outputParameterName + ") -- the ontology does not have an ontologyID URI", transformationURI);
		...
		if (null == r)
			throw new QVTOTransformationURIException("QVTEngine (output parameter: " + outputParameterName + ") -- No output resource created for uri: " + ontologyModelURI, transformationURI);
		saveContents(r, outObjects);
		
		String nsPrefix = ontologyModelURI.trimFileExtension().lastSegment();
		if (null != ontologyPrefixConfigurationProperty) 
			nsPrefix = getConfigurationPropertyStringValueOrDefault(ontologyPrefixConfigurationProperty, nsPrefix);
		...
		try {
			...
			throw new QVTOTransformationURIException("QVTEngine (output parameter: " + outputParameterName + ") -- error while exporting ontology", transformationURI, e);
		} catch (InterruptedException e) {
			throw new QVTOTransformationURIException("QVTEngine (output parameter: " + outputParameterName + ") -- error while exporting ontology", transformationURI, e);
		}
		showMessage("QVTPlugin: ontology model corresponding to output parameter: " + outputParameterName + " saved as: " + ontologyModelURI + " and exported as OWL2 ontology");
		
	}

	public com.nomagic.uml2.ext.magicdraw.classes.mdkernel.Package getTransformationParameterModelElement(String parameterName) {
		
		Bindings engineBindings = qvtoEngine.getBindings(ScriptContext.ENGINE_SCOPE);
		QvtOperationalEvaluationEnv evaluationEnv = (QvtOperationalEvaluationEnv) engineBindings.get(QVTEngineConstants.QVTO_EVALUATION_ENV);
		...
		if (null == modelParameter)
			throw new QVTORuntimeException("There is no transformation model parameter named: " + parameterName);
		...
		if (null == root)
			throw new QVTORuntimeException("No root-level Package element found in model transformation parameter: " + parameterName + " amongst the " + rootElements.size() + " root elements");
		return root;
	}
	...
	public void mdExportPackage(com.nomagic.uml2.ext.magicdraw.classes.mdkernel.Package pkg, File moduleFile) {
		...
		try {
			ProjectsManager projectsManager = Application.getInstance().getProjectsManager();
			ProjectDescriptor moduleDescriptor = ProjectDescriptorsFactory.createProjectDescriptor(moduleFile.toURI());
	...
	/**
	 * Adapted from:
	 * @see org.eclipse.m2m.internal.qvt.oml.ModelExtentHelper.saveContents(Resource, List<EObject>)
	 *
	 * Override as needed for specialized metamodels.
	 * 
	 * @param res the output resource to store the contents to
	 * @param contents the contents of a model resource to save
	 * @throws IOException 
	 */
	...
	/**
	 * Adapted from:
	 * @see org.eclipse.m2m.internal.qvt.oml.ModelExtentHelper.isDynamic(EObject)
	 * 
	 * Override as needed for specialized metamodels.
	 * 
	 * @param eObject an EMF EObject to be saved in a resource
	 * @return whether the eObject is dynamic and requires outputting the schema location of its metamodel resource.
	 */

...
public class JMSMessageListener implements MessageListener {
	...
	public void onMessage(Message msg) {
		try {
			...
			// Get the updated elements which were specified in the
			// incoming message.
			//
			...
			Runnable runnable = new Runnable() {
				public void run() {
					Map<String, ?> projectInstances = ProjectListenerMapping
							.getInstance().get(project);
				...
				private void makeChange(JSONObject ob) {
					Element ele = ExportUtility.getElementFromID((String) (ob)
							.get("sysmlid"));
					if (ele == null) {
						Application.getInstance().getGUILog()
								.log("element not found from mms sync change");
						return;
					}
					String newName = (String) (ob).get("name");
					if (ele instanceof NamedElement && newName != null
							&& !((NamedElement) ele).getName().equals(newName))

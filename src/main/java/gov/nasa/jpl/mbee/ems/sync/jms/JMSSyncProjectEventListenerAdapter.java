...
public class JMSSyncProjectEventListenerAdapter extends ProjectEventListenerAdapter {
    private static final String ERROR_STRING = "[WARNING] Could not connect to JMS. Reverting to offline mode. All changes will be saved in the model until reconnected.";
    ...
    private static boolean initDurable(Project project) {
        ...
        try {
            ...
            e.printStackTrace();
            Application.getInstance().getGUILog().log(ERROR_STRING + " Reason: " + e.getMessage());
            return false;
        }
        String url = jmsInfo != null ? jmsInfo.getUrl() : null;
        if (url == null) {
            Application.getInstance().getGUILog().log(ERROR_STRING + " Reason: Cannot get server URL.");
            return false;
        }
        if (workspaceID == null) {
            Application.getInstance().getGUILog().log(ERROR_STRING + "Reason: Cannot get the server workspace that corresponds to this project branch.");
            return false;
        }
        if (ProjectUtilities.isFromTeamworkServer(project.getPrimaryProject())) {
            String user = TeamworkUtils.getLoggedUserName();
            if (user == null) {
                Application.getInstance().getGUILog().log(ERROR_STRING + " Reason: You must be logged into Teamwork.");
                return false;
            }
        }
        String username = ViewEditUtils.getUsername();
        if (username == null || username.isEmpty()) {
            Application.getInstance().getGUILog().log(ERROR_STRING + " Reason: Could not login to MMS.");
            return false;
        }
        try {
            ConnectionFactory connectionFactory = JMSUtils.createConnectionFactory(JMSUtils.getJMSInfo(project));
            if (connectionFactory == null) {
                Application.getInstance().getGUILog().log(ERROR_STRING + " Reason: Failed to create JMS connection factory.");
                return false;
            }
